...
class MilanChallengerEngine:
    ...
    def _load_data(self):
        # Se il file non esiste (come su Render), lo scarica.
        if not os.path.exists(self.csv_path):
            success = self._download_csv()
            if not success:
                print("ℹ️ Impossibile ottenere il CSV. Il motore userà l'algoritmo sintetico.")
                self.df = None
                return

        try:
            # Carica solo le colonne strettamente necessarie per risparmiare RAM
    ...
    def get_median(self, neighbourhood: str, max_guests: int = None) -> float:
        """Calcola la mediana reale basata sui dati degli host concorrenti."""
